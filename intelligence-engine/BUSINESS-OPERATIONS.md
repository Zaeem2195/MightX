...
- **Brief quality gates** (enforced by the generator, documented in `brief-app/README.md`): ≥3 fully-worked talk tracks (`example=true`, zero `[your X]` placeholders, real competitor + dated signal cited) and ≥2 templates with placeholders for reps; every source across every section must carry a real URL (no `src-nolink` citations); any single underlying signal may appear in at most two sections so the week never looks shallow. Set `BRIEF_STRICT_VALIDATION=1` to make these fatal — the brief is the single most-scrutinised surface in the funnel and a visible quality hole costs you the meeting. Collector-side, procedural 8-K filings (Item 8.01 / 9.01 only) are dropped before analysis so the brief never contains an "we don't know why this was filed" filler paragraph.
